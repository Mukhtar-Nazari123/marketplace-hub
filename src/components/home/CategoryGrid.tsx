          ...
          <div key={category.id} className="mb-8 last:mb-0">
            ...
            <div className={`grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10 gap-3 md:gap-4 ${isRTL ? 'direction-rtl' : ''}`}>
              {category.subcategories?.slice(0, 10).map((sub) => (
                ...
                >
                  {/* Circular Image */}
                  <div className="relative w-14 h-14 sm:w-16 sm:h-16 md:w-20 md:h-20 rounded-[2rem] overflow-hidden bg-gradient-to-br from-primary/10 to-primary/5 border-2 border-transparent group-hover:border-primary/50 transition-all duration-300 group-hover:shadow-lg group-hover:scale-105">
                    {sub.image_url ? (
                      <img
                        src={sub.image_url}
                        alt={sub.name}
          ...
          <div className={`grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-4 ${isRTL ? 'direction-rtl' : ''}`}>
            {categories.map((category) => (
              <Link
                ...
                className="group flex flex-col items-center text-center"
              >
                <div className="relative w-16 h-16 md:w-20 md:h-20 rounded-[2rem] overflow-hidden bg-gradient-to-br from-primary/10 to-primary/5 border-2 border-transparent group-hover:border-primary/50 transition-all duration-300 group-hover:shadow-lg group-hover:scale-105 flex items-center justify-center">
