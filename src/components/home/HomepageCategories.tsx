...
const HomepageCategories = () => {
  ...
  const CategoryItem = ({ item, isSubcategory }: { item: typeof items[0]; isSubcategory: boolean }) => {
    ...
    return (
      ...
      >
        {/* Category Image */}
        <div className="relative w-14 h-14 sm:w-20 sm:h-20 md:w-24 md:h-24 lg:w-28 lg:h-28 rounded-[2rem] overflow-hidden bg-muted border-2 border-transparent group-hover:border-primary/50 transition-all duration-300 group-hover:shadow-lg group-hover:scale-105">
          {displayImage ? (
            <img
              src={displayImage}
              alt={item.name}
