...
export async function saveProduct(options: SaveProductOptions): Promise<SaveProductResult> {
  ...
  try {
    // Generate SKU
    const generatedSKU = generateSKU(formData.categoryId, formData.categoryNameEn || formData.categoryName, formData.name || 'DRAFT', formData.subCategoryNameEn || formData.subCategoryName);
